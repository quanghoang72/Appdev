...
#include <math.h>
#include "sound.h"
// this function will calculate 1 second of sound samples in 8-pieces of slow
// decibel (ISO standard), this data is calculated as 125ms of samples 
...
void sendDATA(short s[]){
	double rms[8],db[8];
	int i,j;
	// calculate 8 pieces of decibel values
	for (i=0;i<8;i++){
		double sum =0.0;
		for(j=0;j<2000;j++){
